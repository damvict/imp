...
def ca_pending_assessment_web(request):
    ...
    email = EmailMessage(
        ...
        cc=["abc.gmail.com","abc.gmail.com","abc.gmail.com"],  # optional
    )
    email.send(fail_silently=False);
...
@login_required
def edit_arrival_notice_view(request, shipment_id):

    shipment = get_object_or_404(Shipment, id=shipment_id)

    if request.method == "POST":
        form = ArrivalNoticeForm(request.POST, instance=shipment)
        ...
        form = ArrivalNoticeForm(instance=shipment)

    return render(
        request,
        "shipments/edit_arrival_notice.html",
        {
            "form": form,
            "shipment": shipment,
        }
...
"""
                ...
                email = EmailMessage(
                    ...
                    cc=["abc.gmail.com","abc.gmail.com","abc.gmail.com","abc.gmail.com","abc.gmail.com"]
                )
...
@login_required
def edit_bank_document_amount_web(request, doc_id):
    """
    Allow the Bank Controller to edit ONLY the BankDocument amount
    before any settlement has been created.

    When the amount is changed:
        1. BankDocument.amount is updated
        2. Related Shipment.amount is updated

    Both changes are performed inside one database transaction.
    """

    document = get_object_or_404(
        BankDocument.objects.select_related("shipment"),
        pk=doc_id
    )

    # ---------------------------------------------------------
    # DO NOT ALLOW EDITING AFTER SETTLEMENT HAS STARTED
    # ---------------------------------------------------------
    if Settlement.objects.filter(document=document).exists():
        return render(
            request,
            "bc/edit_bank_document_amount.html",
            {
                "document": document,
                "error": (
                    "This document cannot be edited because "
    ...
    if request.method == "POST":
        ...
        # -----------------------------------------------------
        # UPDATE BOTH BANK DOCUMENT AND SHIPMENT
        # -----------------------------------------------------
        with transaction.atomic():

            # Update BankDocument amount
            document.amount = new_amount
            document.save(update_fields=["amount"])

            # Get the related Shipment
            shipment = document.shipment

            # Update Shipment amount
            shipment.amount = new_amount
            shipment.save(update_fields=["amount"])
...
def report_dashboard(request):
    ...
    # Document Report
    reports.append({
        "title": "Shipment Age Report",
        "description": "View Shipment listing with Age",
        "icon": "📄",
        "url": "shipment_aging_report",
    })
    ...
    if (
        user.is_superuser
        or user.is_staff
        or user_groups.intersection(management_groups)
    ):
        reports.append({
            "title": "Shipment Phase Report",
            "description": "View shipment progress and phase status within a selected date range.",
            "icon": "🚢",
            "url": "phase_range_report",
        })
...
def shipment_aging_report(request):
    ...
    return render(
        request,
        "reports/shipment_aging_report.html",
        context
    )
from django.shortcuts import render
